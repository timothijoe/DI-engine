import torch
from easydict import EasyDict

if torch.cuda.is_available():
    device = 'cuda'
else:
    device = 'cpu'

env_name = 'game_2048'  # options={'PongNoFrameskip-v4', 'QbertNoFrameskip-v4', 'MsPacmanNoFrameskip-v4', 'SpaceInvadersNoFrameskip-v4', 'BreakoutNoFrameskip-v4', ...}

if env_name == 'PongNoFrameskip-v4':
    action_space_size = 6
    average_episode_length_when_converge = 2000
elif env_name == 'QbertNoFrameskip-v4':
    action_space_size = 6
    average_episode_length_when_converge = 2000
elif env_name == 'MsPacmanNoFrameskip-v4':
    action_space_size = 9
    average_episode_length_when_converge = 500
elif env_name == 'SpaceInvadersNoFrameskip-v4':
    action_space_size = 6
    average_episode_length_when_converge = 1000
elif env_name == 'BreakoutNoFrameskip-v4':
    action_space_size = 4
    average_episode_length_when_converge = 800
elif env_name == 'game_2048':
    action_space_size = 4
    average_episode_length_when_converge = 50
# ==============================================================
# begin of the most frequently changed config specified by the user
# ==============================================================
# collector_env_num = 1
# n_episode = 3
# evaluator_env_num = 1
# num_simulations = 50
# update_per_collect = 1000
# batch_size = 256
# max_env_step = int(1e6)
# reanalyze_ratio = 0.
collector_env_num = 1
n_episode = 3
evaluator_env_num = 1
num_simulations = 5
update_per_collect = 4
batch_size = 5
max_env_step = int(1e6)
reanalyze_ratio = 0.
# ==============================================================
# end of the most frequently changed config specified by the user
# ==============================================================

atari_muzero_config = dict(
    exp_name=f'data_mz_ctree/{env_name[:-14]}_muzero_ns{num_simulations}_upc{update_per_collect}_rr{reanalyze_ratio}_sslw2_seed0',
    env=dict(
        stop_value=int(1e6),
        env_name=env_name,
        obs_shape=(16, 4, 4),
        collector_env_num=collector_env_num,
        evaluator_env_num=evaluator_env_num,
        n_evaluator_episode=evaluator_env_num,
        manager=dict(shared_memory=False, ),
    ),
    policy=dict(
        model=dict(
            observation_shape=(16, 4, 4),
            action_space_size=action_space_size,
            image_channel=16,
            representation_network_type='conv_res_blocks',
            # NOTE: whether to use the self_supervised_learning_loss. default is False
            self_supervised_learning_loss=True,
            frame_stack_num = 1,
            downsample=False,
        ),
        cuda = True,
        device=device,
        collector_env_num=collector_env_num,
        evaluator_env_num=evaluator_env_num,
        env_type='not_board_games',
        game_segment_length=20,
        num_simulations=num_simulations,
        reanalyze_ratio=reanalyze_ratio,
        manual_temperature_decay=False,
        fixed_temperature_value=0.25,
        ssl_loss_weight=2,  # default is 0
        update_per_collect=update_per_collect,
        batch_size=batch_size,
        lr_piecewise_constant_decay=True,
        optim_type='SGD',
        learning_rate=0.2,  # init lr for manually decay schedule
        n_episode=n_episode,
        eval_freq=int(2e3),
        replay_buffer_size=int(1e6),  # the size/capacity of replay_buffer, in the terms of transitions.
    ),
)
atari_muzero_config = EasyDict(atari_muzero_config)
main_config = atari_muzero_config

atari_muzero_create_config = dict(
    env=dict(
        type='game_2048',
        import_names=['zoo.game2048.env.game_env'],
    ),
    env_manager=dict(type='base'),
    policy=dict(
        type='muzero',
        import_names=['lzero.policy.muzero'],
    ),
    collector=dict(
        type='episode_muzero',
        import_names=['lzero.worker.muzero_collector'],
    )
)
atari_muzero_create_config = EasyDict(atari_muzero_create_config)
create_config = atari_muzero_create_config

if __name__ == "__main__":
    from lzero.entry import train_muzero

    train_muzero([main_config, create_config], seed=0, max_env_step=max_env_step)
